...
bool OrtBackend::InferGsj(std::vector<FDTensor> &inputs,
                       std::vector<FDTensor> *outputs, bool copy_to_fd) {
  std::vector<std::vector<FDTensor>> inputs_split;
  std::vector<int64_t> image_shape = inputs[0].Shape();
  for(int i = 0;i < image_shape.size();i++){
    std::cout<<image_shape[i]<<"  ";
  }
  // int channels = image_shape[1]; // 通常是3（BGR）
  int img_width = image_shape[0];
  int img_height = image_shape[1];
  // int block_width = img_width / COLS;
  // int block_height = img_height / ROWS;
  cv::Mat image(img_height, img_width, CV_8UC3, inputs[0].MutableData());
  cv::imwrite("/workspace/rknpu-cpp-python/pingdi_gsj_66.jpg",image);
  return false;
}
