...
int rknnPool<rknnModel, inputType, outputType>::init()
{
    try
    {
        ...
        for (int i = 0; i < this->threadNum; i++)  //初始化推理模型实例类
            models.push_back(std::make_shared<rknnModel>(this->modelPath.c_str()));
    }
    catch (const std::bad_alloc &e) //防止内存不足
    {
...
int rknnPool<rknnModel, inputType, outputType>::put(inputType inputData)
{
    ...
    futs.push(pool->submit(&rknnModel::infer, models[this->getModelId()], inputData.img,inputData.id));  //这里并没有保证了有序性，只是随机分配，还需要考虑
    return 0;
}
