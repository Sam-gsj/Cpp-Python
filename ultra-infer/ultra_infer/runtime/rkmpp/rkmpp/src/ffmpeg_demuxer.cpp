...
namespace FFHDDemuxer{
    ...
    static inline bool check_ffmpeg_retvalue(int e, const char* call, int iLine, const char *szFile) {
        if (e < 0) {
            PLOGE << "FFMPEGDemuxer error " << call << ", cdoe = " << e << " in file " << szFile << ":" << iLine;
            return false;
        }
        return true;
    }
                ...
                *pnVideoBytes = m_pktFiltered->size;
                ...
                if (m_bMp4MPEG4 && (m_frameCount == 0)) {
                    ...
                    if (extraDataSize > 0) {
                        ...
                        if (!m_pDataWithHeader) {
                            PLOGE << "FFmpeg error, m_pDataWithHeader alloc failed";
                            return false;
                        }
    ...
    private:
        ...
        bool open(AVFormatContext *fmtc, int64_t timeScale = 1000 /*Hz*/) {
            if (!fmtc) {
                PLOGE << "No AVFormatContext provided.";
                return false;
            }
            ...
            if (m_iVideoStream < 0) {
                PLOGE << "FFmpeg error: Could not find stream in input file";
                return false;
            }
            ...
            case AV_PIX_FMT_YUVJ444P:   // jpeg decoder output is subsampled to NV12 for 422/444 so treat it as 420
                ...
                break;
            default:
                PLOGW << "ChromaFormat not recognized. Assuming 420";
                m_nBitDepth = 8;
                m_nChromaHeight = (m_nHeight + 1) >> 1;
                m_nBPP = 1;
            }
            ...
            if (m_bMp4H264) {
                ...
                if (!bsf) {
                    PLOGE << "FFmpeg error: av_bsf_get_by_name() failed";
                    return false;
                }
        ...
        AVFormatContext *CreateFormatContext(const string& uri) {
            ...
            if (string_begin_with(uri, "rtsp://")){
                ...
                av_dict_set(&options, "max_delay", "10000", 0); /* 设置最大时延，单位微秒 */
            }
