...
DeepGlobeRKNN::DeepGlobeRKNN(const std::string& model_path)
    : m_model_path(model_path), m_ctx(0), m_input_attrs(nullptr), 
      m_output_attrs(nullptr), m_model_w(0), m_model_h(0), m_is_init(false) {
    m_class_names = {
...
int DeepGlobeRKNN::Init() {
    ...
    if (m_input_attrs[0].fmt == RKNN_TENSOR_NCHW) {
        ...
        m_model_w = m_input_attrs[0].dims[3]; 
    } else {
        m_model_h = m_input_attrs[0].dims[1];
...
std::vector<cv::Mat> DeepGlobeRKNN::Predict(const cv::Mat& src_img) {
    ...
    if (m_output_attrs[0].fmt == RKNN_TENSOR_NCHW) {
        ...
        for (int i = 0; i < m_model_h; i++) {
            for (int j = 0; j < m_model_w; j++) {
                float max_val = -1e10;
                int max_cls = 0;
                int pixel_offset = (i * m_model_w + j) * num_classes;
                for (int k = 0; k < num_classes; k++) {
                    float val = output_data[pixel_offset + k];
                    if (val > max_val) {
                        max_val = val;
                        max_cls = k;
                    }
